(ns eugene.bakisov.scramblies.web.middleware.exception
  (:require
   [clojure.tools.logging :as log]
   [reitit.ring.middleware.exception :as exception]))

(defn handler [message status ^java.lang.Object exception request]
  (when (>= status 500)
    (log/error exception))
  {:status status
  ...
  (exception/create-exception-middleware
    (merge
      ...
      {:system.exception/internal     (partial handler "internal exception" 500)
       ...
       ::exception/wrap               (fn [handler-fn e request]
                                        (handler-fn e request))})))
